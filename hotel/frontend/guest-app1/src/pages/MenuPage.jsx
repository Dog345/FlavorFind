...
export default function MenuPage() {
  ...
  const handleQuickAdd = (item) => {
    addItem(item, null, [], 1, '', item.price);
    toast.success(`${item.name} added to cart`, { icon: '🛒' });
  };

  return (
    <>
      <main className="page-container pb-6">
        <div className="pt-4 pb-3 sticky top-[60px] md:top-[64px] z-20 bg-cream md:max-w-sm">
        ...
        {isSearching ? (
          ...
          <>
            <div className="sticky top-[112px] md:top-[116px] z-20 bg-cream pb-2">
              ...
              <>
                {popularItems.length > 0 && (
                  <section className="mt-3">
                    ...
                    <div className="flex gap-3 overflow-x-auto -mx-4 px-4 md:-mx-8 md:px-8 lg:-mx-10 lg:px-10 md:grid md:grid-cols-4 lg:grid-cols-5 md:gap-4 md:overflow-visible">
                      {popularItems.map((item) => (
                        <button
                          key={item.id}
                          onClick={() => setOpenItem(item)}
                          className="tap-shrink w-36 md:w-full shrink-0 text-left rounded-xl2 bg-cream-card shadow-card overflow-hidden"
                        >
                          <img
                            src={item.image_url}
                            alt=""
                ...
                {categories.map((cat) => (
                  <section key={cat.id} id={`category-${cat.id}`} className="mt-6 scroll-mt-28 md:scroll-mt-32">
...
function SearchResults({ results, loading, onOpen, onQuickAdd }) {
  if (loading) {
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 mt-3">
  ...
  return (
    <>
      <div className="mt-3">
        ...
        <div className="flex md:grid md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4 overflow-hidden">

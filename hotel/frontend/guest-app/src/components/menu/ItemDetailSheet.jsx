...
export default function ItemDetailSheet({ item, open, onClose }) {
  ...
  const unitPrice = useMemo(() => {
    ...
    const base = variant ? variant.price_adjustment + item.base_price : (item.base_price ?? item.price);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="w-full" style={{ minHeight: '50vh' }}>
        <img src={item.image_url} alt={item.name} className="w-full h-[42vh] object-cover" />
      </div>
      ...
      <div className="px-5 pt-4 pb-32">
        ...
        <div className="flex items-center gap-2 mt-2">
          {(item.prep_time_min ?? item.prep_time_minutes) > 0 && (
            <span className="flex items-center gap-1 text-[12px] text-ink-soft">
              <Clock size={13} /> {item.prep_time_min ?? item.prep_time_minutes} min
            </span>
          )}
        </div>
      ...
      <div
        className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] p-4 bg-cream-card border-t border-line"
